  ...
  return (
    <figure className={`moviecard ${isVis && 'moviecard_visible'}`}>
      <div className='moviecard__conteiner'>
        ...
        {pass === 'Movies'
          ? (<button className={`btn moviecard__btn ${isSaved && 'moviecard__btn_active'}`} onClick={isSaved? handleDelete: handleSave} type="button">Сохранить</button>)
          : (<button className='btn moviecard__btn moviecard__btn_saved' type="button" onClick={handleDelete} ></button>)}
      </div>
